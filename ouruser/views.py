...
def register(request):
    if request.method == 'POST':
        if request.POST['password'] == request.POST['re-password']:
            user = User.objects.create_user(username = request.POST['username'], password =request.POST['password'], useremail = request.POST['useremail'], userschool= request.POST['userschool'])
...
def delete(request, pk):
    ...
    if request.method == "POST":
        ...
        return render(request, 'delete_confirm.html', {'object': blog})
...
def search(request):
    ...
    sch = Blog.objects.all()
    if query :
        sch_boards = sch.filter(title__contains=query)
        return render(request, 'search.html', {'sch_boards': sch_boards})